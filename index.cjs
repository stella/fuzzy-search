...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasi")) {
    ...
    try {
      candidateError = __napiWasiResolveCandidate('./fuzzy-search.wasi.cjs', false, ["./fuzzy-search.wasm32-wasi.debug.wasm","./fuzzy-search.wasm32-wasi.wasm"])
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasi")) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('@stll/fuzzy-search-wasm32-wasi', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('@stll/fuzzy-search-wasm32-wasi/package.json').version
          if (bindingPackageVersion !== '1.1.3') {
            throw new Error(`WASI binding package version mismatch, expected 1.1.3 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`)
          }
        }
